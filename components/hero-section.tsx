      ...
      >
        ...
        <div
          className={cn(
            ...
            isArabic ? "md:order-2 md:items-end md:text-right" : "md:order-1 md:items-start md:text-left", // Keep existing alignment for headline/bullets
          )}
        >
          ...
          <ul className="mt-4 space-y-3 text-lg md:text-xl">
            {currentContent.hero.subtext.map((point, index) => {
              ...
              return (
                <li key={index} className={cn("flex items-center gap-3")}>
                  {" "}
                  {/* Keep existing icon alignment */}
                  {IconComponent && <IconComponent className="h-6 w-6 text-accent-teal flex-shrink-0" />}
                    ...
                    {point.text.split(point.highlight).map((part, i) => (
                      <React.Fragment key={i}>
                        ...
                        {i < point.text.split(point.highlight).length - 1 && (
                          <span className="text-accent-teal font-bold">{point.highlight}</span>
                        )}
                      </React.Fragment>
                    ))}
                  </span>
          ...
          <Link
            ...
            className={cn("mt-8 block", isArabic ? "md:self-start" : "md:self-start")} // UPDATED: Use self-start for left alignment
            onClick={(e) => {
